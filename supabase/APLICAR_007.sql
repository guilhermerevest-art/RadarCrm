...
-- 1) Função helper: retorna tenant do user logado (bypass RLS via SECURITY DEFINER)
...
-- 2) Função helper: retorna papel do user no tenant
...
CREATE POLICY "globais_update_system" ON radar_obras_globais
...
CREATE POLICY "pontuacao_update_system" ON radar_user_pontuacao
  FOR UPDATE USING (auth.uid() IS NOT NULL);
