...
BEGIN
  FOR r IN
    ...
    WHERE schemaname = 'public' AND tablename IN (
      'radar_fontes_config','radar_ingestao_jobs','geocoding_cache'
    )
  LOOP
...
-- Resolve endereco para lat/lng usando cache -> Mapbox -> Nominatim
-- Pre-requisito: extensao http (ja incluida no Supabase)
...
COMMENT ON FUNCTION fn_geocoding_resolver IS
  'Resolve endereco para lat/lng. Cache > Mapbox > Nominatim. Retorna {lat,lng,precisao,provider} ou {error}. Pre-req: http extension.';

-- 5. Funcao fn_radar_obras_no_raio
-- Busca obras num raio a partir de lat/lng usando PostGIS
...
BEGIN
  RETURN QUERY
    SELECT
      ...
      o.lat,
      o.lng,
      COALESCE(g.fase_consolidada, o.fase_atual)::TEXT AS fase_atual,
      o.porte,
      o.valor_estimado,
      ROUND(
        (6371 * acos(
      ...
      o.qualidade_score,
      COALESCE(g.total_marcacoes, 0) AS total_marcacoes,
      g.fase_consolidada,
      o.fonte,
      o.obra_global_id
    FROM radar_obras o
...
COMMENT ON FUNCTION fn_radar_obras_no_raio IS
  'Busca obras num raio a partir de lat/lng usando PostGIS ST_DWithin.';

-- 6. Funcao fn_ingestao_cno_processar
-- Processa dados do CNO e insere/atualiza radar_obras_globais e radar_obras por tenant
...
COMMENT ON FUNCTION fn_ingestao_cno_processar IS
  'Processa dados do CNO e insere/atualiza radar_obras_globais e radar_obras por tenant.';
